...
HBA_MEM *ahci_get_abar(uint8_t bus, uint8_t slot, uint8_t func) {
    pci_standard_device_header_t ahci_ctrl_header =
        pci_get_standard_device_header(bus, slot, func);
    if (ahci_ctrl_header.base_header.vendor == 0xFFFF)
        return 0;
...
static int check_type(HBA_MEM *abar, HBA_PORT *port, int port_num) {
    ...
    case 2: // Partial - device is sleeping but wake up is fast
        if (!ahci_wakeup_port(abar, port))
            return AHCI_DEV_WAKEUP_FAIL;
        break;
...
int ahci_wakeup_port(HBA_MEM *abar, HBA_PORT *port) {
    ...
    for (int i = 100; i > 0; --i) {
        ...
        if (current_det == HBA_PORT_DET_PRESENT &&
            current_ipm == HBA_PORT_IPM_ACTIVE) {
            // Clear ERR
            port->serr |= 0xFFFFFFFF;
            return 1;
        }
